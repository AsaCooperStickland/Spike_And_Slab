...
class Testing(object):
    ...
    def get_data(self, size = 3):
        ...
        Y_0_test = (0.1*X_test[:, 1] + 0.0*X_test[:, 4] +
           np.random.normal(scale = 0.01, size = test_size))
        Y_1_test = (0.3*X_test[:, 1] - 0.0*X_test[:, 4] +
           np.random.normal(scale = 0.01, size = test_size))
        Y_2_test = (-0.5*X_test[:, 1] - 0.0*X_test[:, 4] +
           np.random.normal(scale = 0.01, size = test_size))
        Y_3_test = (0.2*X_test[:, 1]  - 0.0*X_test[:, 4] +
           np.random.normal(scale = 0.01, size = test_size))
        ...
        for size, i in zip(self.sizes, np.arange(0, self.number_sizes)):
            ...
            if self.gen_weights == True:
                sns = spike_n_slab.run_MCMC(self.X_train, self.y_train, 0.02,
                                            ...
                                            prior = 'spike_n_slab',
                                            save_message = 'w2eights' + str(size),
                                            samples = 10000)
            sns_weights = np.genfromtxt(
                                'spike_slab_results\s_n_s_weights\w2eights'\
                                + str(size) + '.csv')
            if self.gen_weights == True:
                gauss = spike_n_slab.run_MCMC(self.X_train, self.y_train, 0.02,
                                              0.2, weights, zs, 0.5, 4, 100.0,
                                              prior = 'gauss',
                                     save_message = 'g2auss_weights' + str(size),
                                              samples = 10000)
            gauss_weights = np.genfromtxt(
                'spike_slab_results\s_n_s_weights\g2auss_weights'\
                                + str(size) + '.csv')
...
if new_data == True:
    versus = Testing(sizes = [16, 20], dim = 5, runs = 5)
    mle_std, sns_std, \
...
p0 = np.genfromtxt('spike_slab_results\s_n_s_p0\w2eights20.csv')
...
sig2 = np.genfromtxt('spike_slab_results\s_n_s_sigma2\w2eights20.csv')
